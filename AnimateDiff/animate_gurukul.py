...
MODEL_CONFIGS = {
    ...
    "artistic": {
        "base_model": "xyn-ai/anything-v4.0",  # Use anime model for artistic style
        "vae_model": "stabilityai/sd-vae-ft-mse",
        "motion_adapter": "guoyww/animatediff-motion-adapter-v1-5-2"  # Use stable motion adapter
    }
}
...
def generate_clip(prompt: str,
    ...
    # Style-specific negative prompts - CRITICAL FIXES FOR SPECIFIC ISSUES
    if style == "anime":
        # ANIME NEGATIVE PROMPTS - CRITICAL FIXES FOR CLARITY & BACKGROUNDS
        negative_prompt = "blank background, empty background, white background, void background, no background, missing background, plain background, bare background, solid color background, monochrome background, unclear movement, confusing action, unclear what happening, different character each clip, changing character, different person, character inconsistency, new character, character change, appearance change, not following prompt, ignoring story, wrong scene, incorrect action, color rays, light artifacts, rainbow effects, lens flare, light streaks, no character, empty scene, missing character, invisible character, no person, indoor, inside, room, wall, ceiling, boy, male, man, masculine, static pose, standing still, no movement, no action, inactive, motionless, frozen pose, still pose, not moving, stationary, idle, passive, cropped body, half body, cut off, partial body, missing legs, missing arms, close up, zoomed in, tight crop, head only, upper body only, torso only, incomplete figure, deformed face, distorted face, ugly face, bad face, malformed face, disfigured face, bad anatomy, deformed body, distorted body, malformed body, broken anatomy, extra limbs, missing limbs, bad hands, deformed hands, extra fingers, missing fingers, blurry, low quality, bad quality, poor quality, worst quality"
        ...
        # ARTISTIC NEGATIVE PROMPTS - CRITICAL CONSISTENCY & BACKGROUND FIXES
        negative_prompt = "blank background, empty background, no background, void background, missing background, plain background, bare background, solid color background, monochrome background, inconsistent character, changing character, different character each clip, character inconsistency, different person, facial inconsistency, different facial features, face change, identity change, appearance change, new character, character change, indoor, inside, room, wall, ceiling, boy, male, man, masculine, static pose, standing still, no movement, no action, inactive, motionless, frozen pose, still pose, not moving, stationary, idle, passive, different hair, different clothes, different outfit, cropped body, half body, cut off, partial body, missing legs, missing arms, close up, zoomed in, tight crop, head only, upper body only, torso only, incomplete figure, deformed face, distorted face, ugly face, bad face, malformed face, disfigured face, bad anatomy, deformed body, distorted body, malformed body, broken anatomy, extra limbs, missing limbs, bad hands, deformed hands, extra fingers, missing fingers, blurry, low quality, bad quality, poor quality, worst quality"
    ...
    if style == "realistic":
        ...
        inference_steps = 45  # Increased for better character generation
    else:  # artistic
        guidance_scale = 18  # Reduced for better facial stability
        inference_steps = 42  # Increased for better facial consistency
