"""
Gurukul LoRA Training - 1 EPOCH TEST for time estimation
"""
import os
import time
...
class GurukulDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        caption = self.captions.get(self.images[idx].name, "Educational scene")
        return img, caption
...
def train():
    ...
    print(f"\n{'='*70}")
    print(f"🧪 1 EPOCH TEST - Time Estimation for 100 Epochs")
    print(f"{'='*70}")
    print(f"Dataset size: {len(dataset)} images")
    print(f"Batch size: 1")
    print(f"Total steps: {len(loader)}")
    print(f"{'='*70}\n")
    ...
    print(f"\nTraining 1 epoch for time estimation")
    print("="*70 + "\n")
    ...
    # Train 1 epoch
    losses = []
    pbar = tqdm(loader, desc=f"Epoch 1/1")
    ...
    print(f"{'='*70}")
    print(f"1 Epoch Time: {epoch_time/60:.2f} minutes ({epoch_time:.1f} seconds)")
    print(f"Average Loss: {avg_loss:.4f}")
    print(f"Steps Completed: {len(losses)}")
    print(f"\n{'='*70}")
    print(f"📊 ESTIMATES FOR 100 EPOCHS")
    print(f"{'='*70}")
    
    estimated_100_hours = (epoch_time * 100) / 3600
    estimated_100_days = estimated_100_hours / 24
    
    print(f"Estimated Time: {estimated_100_hours:.2f} hours ({estimated_100_days:.2f} days)")
    print(f"")
    
    # Cost estimates for cloud GPUs
    print(f"💰 CLOUD GPU COST ESTIMATES (100 epochs):")
    print(f"   RTX 3060 Ti (current): {estimated_100_hours:.1f} hours (your local GPU)")
    
    # L40 is ~2.5-3x faster
    l40_hours = estimated_100_hours / 2.75
    l40_cost = l40_hours * 1.00  # $1.00/hr average
    print(f"   L40 (48GB): ~{l40_hours:.1f} hours (~${l40_cost:.0f} @ $1.00/hr)")
    
    # A100 is ~3-3.5x faster
    a100_hours = estimated_100_hours / 3.25
    a100_cost = a100_hours * 1.50  # $1.50/hr average
    print(f"   A100 (40GB): ~{a100_hours:.1f} hours (~${a100_cost:.0f} @ $1.50/hr)")
    
    print(f"\n{'='*70}")
    print(f"✅ Test complete! No model saved (test only)")
    print(f"{'='*70}")
